...
const UpdateProduct = () => {
  ...
  useEffect(() => {
    ...
    if (id) {
      adminStore.fetchProduct(id).then((result: Product) => {
        if (result) {
          setName(result.name as string)
          setOldPrice(`${result.old_price}`)
          setNewPrice(`${result.new_price}`)
          setShortDescription(result.short_description as string)
          setDescription(result.description as string)
          setFeature(result.feature as string)
          setImage(result.image as string)
          setCategoryID(result.categoryId as number)
        }
      })
    }
  }, []);
  ...
  const onUpdateProduct = async () => {
    ...
    await adminStore.updateProduct(product)
    adminStore.products[adminStore.products.findIndex((item) => item.id === product.id)] = product
    ...
    <div>
      ...
      <div className="flex mx-[-16px]">
        ...
        <div className="w-7/12 px-4">
          ...
          <div className="flex mx-[-16px]">
            <div className="mb-6 w-6/12 px-4">
