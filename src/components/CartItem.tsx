...
function CartItem({ product }: { product: Cart }) {
  ...
  const changeQuantiy = async (type: string) => {
    if (type === 'plus') {
      ...
      cartStore.Set(product.product_id as number, quantity + 1)
    } else {
      setQuantity(quantity - 1)
      cartStore.Set(product.product_id, quantity - 1)
    }
  }
  ...
  return (
    <div className="w-full flex p-4 rounded-lg shadow-md relative bg-white mb-4">
      ...
      <div className="w-8/12">
        ...
        <div>
          <span className="text-red-500 font-semibold text-base mr-3">{formatCurrency(product.new_price)}</span>
          <span className="text-gray-400 mr-2">{formatCurrency(product.old_price)}</span>
      ...
      <div className="absolute right-[12px] cursor-pointer" onClick={() => cartStore.Remove(product.product_id)}>
        <img src={CloseIcon} />
      </div>
    </div>
  )
